...
function initHeart() {
  ...
  class HeartParticle {
    constructor(t) {
      ...
      this.trail  = [];
      this.trailLen = Math.floor(2 + Math.random() * 3);
    }
  ...
  function buildParticles() {
    const N = Math.min(96, Math.floor(W * 0.08));
  ...
  function draw(time) {
    ...
    beatPhase += 0.012;
    const beat = 1 + 0.008 * Math.sin(beatPhase);
  ...
  if (!document.getElementById('floatKF')) {
    ...
    style.textContent = `
      @keyframes floatUp {
        ...
        5%   { opacity: 1; }
        90%  { opacity: 0.4; }
        100% { transform: translateY(-55vh) scale(0.5); opacity: 0; }
      }
    `;
...
function initRSVP() {
  ...
  form.addEventListener('submit', e => {
    ...
    if (!hasError) {
      form.style.transition = 'opacity 0.18s, transform 0.18s';
    ...
    if (!ticking) {
      requestAnimationFrame(() => {
        ...
        if (y < window.innerHeight * 1.5) {
          ...
          if (heart) heart.style.transform = `translateY(${y * 0.04}px)`;
        }
        ticking = false;
      });
      ticking = true;
